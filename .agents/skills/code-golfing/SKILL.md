...
- After exit faces needed their exact identity, attaching the polygon corner to the already-returned normal instead of returning a separate face index saved 12 bytes. Returning the shared corrected sheet as `[start, span, side]` rather than a named object, and locating its first edge point instead of returning its midpoint, saved another 14 bytes. Summing each exit point and then subtracting its entry point saved 2 more, and keeping the original `width` constant with an absolute growth ratio saved 1: 29 bytes together.
- Collapsing the sheet correction into one signed scale cost 11-13 bytes, reordering it into a signed narrow-growth branch cost 15, and squared width comparisons cost 21. Caching the duplicated traced `Path2D` cost 4 bytes. Moving the face identity onto a ray's `away` or exit-point vector cost 19-20 bytes, and moving it to the ray root cost 1.
