...
import { replacePostTerser } from './replace-post-terser.js';
...
export function viteJs13kPre(flags = {}) {
  return {
    ...
    transform(src, id) {
      if (/\.js$/.test(id)) {
        return {
          code: replacePreTerser(stripIfdef(src, flags)),
          map: null,
        };
      }
    },
      ...
      for (const name of htmlFiles) {
        ...
        for (const jsName of jsAssets) {
          ...
          if (jsChunk.code != null) {
            const scriptCode = replacePostTerser(jsChunk.code);
            ...
            if (buildLevel === 'search') {
              ...
              replacedHtml = await replaceScript(replacedHtml, jsChunk.fileName, scriptCode);
            }
          }
        }
