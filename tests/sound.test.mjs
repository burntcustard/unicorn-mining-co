...
assert.equal(Math.round(first.stopTimes[0] * 100) / 100, 1.01, 'native stop is scheduled for the end of the 1s fade');
...
assert.equal(idle.gain.value, 1, 'biting rises to full volume');
...
assert(Math.abs(engine.gain.value - .144) < 1e-8, 'full thrust retains its master level');
