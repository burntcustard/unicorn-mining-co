...
const randomResource = (random: () => number) =>
  3 - Math.floor(random() ** 3 * 4);

// Always turning one way or the other, and never slowly enough to look stopped
const randomSpin = (random: () => number) =>
  ((random() < 0.5 ? -1 : 1) * (1 + random())) / 40;
...
const makeAsteroids = (
  ...
  const avoid = worldObjects.filter(
    (object) =>
      object.position.distanceTo(field.position) <
      field.fieldRadius + object.radius,
  );
...
export const generateWorld = (seed: number) => {
  const random = seededRandom(seed);

  const stations = distribute(
    Array.from({ length: 24 }, () => ({
      radius: 400,
      spin: randomSpin(random),
    })),
    { density: 8000, radius: worldRadius },
    [],
    random,
  );

  const wrecks = distribute(
    Array.from({ length: 32 }, () => ({
      radius: 100,
      spin: randomSpin(random),
    })),
    { density: 5500, radius: worldRadius },
    [],
    random,
  ).sort((a, b) => b.position.length() ** 2 - a.position.length() ** 2);

  const fields = distribute(
  ...
  const wreckFields = wrecks.map((wreck) => {
    const { position } = wreck;
    const field = clueFields.reduce((nearest, candidate) =>
      position.distanceTo(candidate) < position.distanceTo(nearest)
        ? candidate
        : nearest,
    );
  ...
  const clueWrecks = new Set(
    clueFields.flatMap((field) =>
      wreckFields
        .filter((wreckField) => wreckField.field === field)
  ...
  wreckFields.forEach((wreckField, index) => {
    ...
    wreck.shades =
      [colors.yellow, colors.green, colors.cyan, colors.red][index] ||
      colors.orange;
    ...
    if (hasClue) {
      wreck.message = `${field.resource === 1 ? 'AMETHYST CLUSTER' : 'GOLD ORE'} ${Math.round(field.position.x)}/${Math.round(field.position.y)}`;
    }
  });
