...
const randomResource = (random: () => number) => 3 - Math.floor(random() ** 3 * 4);

// Always turning one way or the other, and never slowly enough to look stopped
const randomSpin = (random: () => number) => (random() < 0.5 ? -1 : 1) * (1 + random()) / 40;

const makeAsteroids = (field: any, worldObjects: any[], random: () => number) => {
  ...
  const avoid = worldObjects.filter((object) =>
    object.position.distanceTo(field.position) < field.fieldRadius + object.radius);
