...
import { type Contact as PhysicsContact } from './dynamics/Contact';
...
import './collision/shape/CollideCirclePolygon';
import { Vec2 } from './common/Vec2';
import { physicsScale, contactSpeedThreshold } from './game-settings';
...
import { sweepSensors } from './sweep-sensors';
...
export class GamePhysics {
  ...
  constructor() {
    this.world.on('pre-solve', (contact) => {
      ...
      if (impact > (this.impacts.get(contact)?.impact || 0))
  ...
  step({
    ...
    // Sensors do not participate in Planck's solid-body CCD. Sweep them too,
    // including rotation, so a tiny drill/cargo throat cannot skip a target.
    this.contacts.push(
      ...sweepSensors({ records: [...this.bodies.values()], previous }),
    );
  ...
  private sync(entity: GameObject) {
    ...
    const geometry = JSON.stringify(
      [
        ...
        shapes,
        colliders.map((c) => [c.physics === false, c.collisionMargin]),
      ],
      (_, value) =>
    ...
    if (geometry !== record.geometry) {
      ...
      record.fixtures = shapes.map((shape, index) => {
        ...
        return record!.body.createFixture(fixtureShape, {
          ...
          friction: 0,
          isSensor: colliders[index].physics === false,
