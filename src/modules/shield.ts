...
export const shield = {
  ...
  model: [
    {
      lines: ({ phase }: { phase: number }) => [
        crossLine(phase),
        crossLine(phase + Math.PI / 2),
      ],
      path: () => dial,
      radius: () => radius,
    },
    {
      ...
      // it on the way down
      path: ({ activationProgress }: { activationProgress: number }) =>
        activationProgress && circlePath(bubbleRadius * activationProgress),
      ...
      radius: ({ activationProgress }: { activationProgress: number }) =>
        bubbleRadius * activationProgress,
      fillAlpha: 2,
    },
  ],
  price: 900,
  // A quarter turn brings the cross back around to where it started
  update: (segment: any, dt: number) => {
    segment.phase =
      (segment.phase + dt * spinRate * segment.activationProgress) %
      (Math.PI / 2);
  },
  unhurtWhen: 1,
  zIndex: 1,
