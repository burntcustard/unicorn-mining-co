 ...
 */
const fluteLines = ({ phase }: { phase: number }) =>
...
export const horn = {
  ...
  bounciness: (segment: any) =>
    segment.activationProgress > 0.5 ? grindBounce : 0,
  ...
  model: [
    {
      lines: fluteLines,
      points: [
        [hornBase, -hornHalfWidth],
        [hornBase + hornLength, 0],
        [hornBase, hornHalfWidth],
      ],
    },
  ],
  ...
  update: (segment: any, dt: number) => {
    segment.phase =
      (segment.phase + dt * spinRate * segment.activationProgress) % 1;

    const active =
      segment.active && (segment.mount ? segment.mount.health > 0 : true);
