...
const cargoOf = (ship: any) => [
  ...[...ship.cargoBay, ...ship.cargo.map(({ item }: any) => item)].reduce(
    (types, item) => types.set(item, (types.get(item) || 0) + 1),
    new Map(),
  ),
];
...
const fitsOf = (ship: any, mount: any) =>
  mount.fits.flatMap((type: any) => {
    const owned = ship.modules.filter(
      (module: any) =>
        module.oneOf === type && (!module.mount || module.mount === mount),
    );
...
const selectionOf = (ship: any) => {
  ...
  const actions =
    stage > 1
      ...
      : [];
  const swatches =
    stage > 1 && (hullMenu || currentModule?.oneOf) ? paints : [];
 ...
 */
export const moveSelection = (delta: number, ship: any, sub: number) => {
  playSound(8);
...
export const confirmSelection = (ship: any) => {
  ...
  if (picked === 'SELL') {
    ...
    ship.modules = ship.modules.filter((module: any) => module !== item);
    ship.cargo = ship.cargo.filter((cargoItem: any) => cargoItem.item !== item);
...
export const renderDocked = (game: any, ship: any) => {
  ...
  menu.forEach((item: any, i: number) => {
    ...
    const shades = (item === 'HULL' ? ship : item.oneOf ? item : item.module)
      ?.shades;
