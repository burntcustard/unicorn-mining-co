...
import { type GameState, type Module, type Segment } from '../types';
...
export const renderControls = (game: GameState, ship: Ship) => {
  ...
  ship.mounts.forEach(
    ({ module }: { module?: Module | 0 }) =>
      ...
      !modules.includes(module.oneOf) &&
      modules.push(module.oneOf),
  );
  ...
  modules.forEach((module, i) => {
    ...
    if (
      ship.segments.some(
        (seg: Segment) => seg.module.oneOf === module && seg.active,
      )
    ) {
