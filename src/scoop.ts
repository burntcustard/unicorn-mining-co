 ...
 */
export const scoop = (contacts: Contact[]) => {
  contacts.forEach(({ collider, other }) => {
    ...
    if (!item.item || !hitbox.segment?.catches || !game.sprites.includes(item))
      return;

    const craft = hitbox.owner as import('./ship').Ship;
    ...
    if (item.position.distanceTo(hitbox.position) > hitbox.radius) return;

    if (item.message) {
      // Read on the way in and thrown away after, so a note never costs a
      // hold anything to carry
      if (!item.unlock || !unlockColor(item.unlock, 'CARGO FOUND'))
        say(item.message);
    } else {
